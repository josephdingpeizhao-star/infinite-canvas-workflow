...
>
> 最后更新：2026-07-26（MU-02 工具栏“上传素材”多选已收官：fork 实现+全量自动验证+用户真人画布验收四项全过。上传后仍由用户连线决定素材角色，不自动连线、不自动登记，也不新增上传阶段的 SHA 比较门禁。）
...
批次生命周期新增账本态 `batch_recycled` / `batch_restored`。事件账本与 batch manifest 永远留在主仓；回收只把 manifest 声明的工作区顶层目录通过同卷单次 `os.rename` 移入同级 `_回收站`。不提供自动物理删除，不使用会回退逐文件复制的 move/copy 路径。还原仅由 CLI 发起，先搬回目录、后追加恢复事件，且不自动重建画布节点。

生产、QC 单图返修、交付和关账四个真实入口统一在首次写入前读取同一生命周期函数；旧 `--serve` 与 manifest 投影、配置写回和风格补登也纳入冻结边界。回收态优先于关账态；还原只解除回收限制，原已关账批次仍保持关账。建批不读取回收状态，因为永不删除的 manifest 已使批次号终身不可复用；demo 继续只认 `.canvas_demo`，不接真实批次门禁。只读助手先读主仓事件，已回收时只回答账本与 manifest 状态，不解析已搬走的 QC、交付或工作区文件。
