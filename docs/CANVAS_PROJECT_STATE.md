...
>
> 最后更新：2026-07-21（第三次闸门①仍在产品识别空答复后完成专项离线 TDD：Canvas Agent 的正常完成通知与备用失败通知都会保留固定脱敏失败码，主仓两路均能显示明确停机文案；对象状态不再显示 `[object Object]`，0/1/2 张本地图片输入协议保持不变。主仓 402 项、fork 61 项/332 断言、Canvas Agent 构建、TypeScript 与生产构建通过。未重跑闸门、未触碰用户画布或批次数据，零网络、零模型、零密钥、零费用）。
...
| M2-b | 真图流式上桌 + 真实费用闸门（NC-02） | ✅ 阶段 C/D 纯离线实现；✅ 僵尸工作台与空答复通知链/重复命令最小修复完成；⏸️ 等现场验收后重跑闸门①完整预检 | 本轮主仓与 fork 独立提交 | 主仓 402/402；fork 61 项/332 断言；正常/备用通知的空答复失败码、无空产物、脱敏诊断事件、0/1/2 图输入和同页单次提交均已 TDD 覆盖；零网络、零模型、零密钥、零费用，真实用户画布与两个批次未触碰 |
...
- **改动登记册：`FORK_NOTES.md`（fork 仓库根目录）**——截至 2026-07-21 共 37 个锚点：#5–#8、#36 为 canvas-agent 兼容；#10–#19 为 M1；#20–#26 为 M2-a；#27–#35、#37 为 M2-b 类型、页面接入、机器/信息卡人话 UI、健康预检、单页闸门锁与用户文档。动 fork 前必读，同步上游后逐条复核。
- M2-b 在 fork 新增真实费用、生产状态、正式图片转存、风格补登三套前端合同与页面私有控制器；补登按钮先做 17373 健康预检，失败不发号、不计时、不重试。真实费用确认按同一页面的机器/批次只允许一次提交。Canvas Agent 的通用完成判定要求本轮非空助手答复，空答复返回固定脱敏码。工作流和信息卡只显示用户可见状态，不包含九工序业务实现；零新增依赖。fork 全量现为 55 项、332 个断言。
...
3. **M2 通真电（严格按 a→e 推进）**：
   ...
   2. **M2-b 真图 + 费用闸门（阶段 C/D 与两轮离线稳定性修复已完成，阶段 E 待重新验收）**：真实费用卡、风格补登、原三段门禁编排、正式图片逐张上桌、浏览器持久化、空答复失败和同页单次提交均已离线实现；再次真实调用须另获批准。
