...
- 真实执行默认关闭；只有获得用户明确批准后，才可在该次服务进程中临时设置 `CODEX_DEV_ALLOW_REAL_EXECUTION=1`，该开关不写配置、不持久化。既有 identity/style/angle/main 真实验收历史、`ExecutionRequest / ExecutionResult`、三段门禁、默认 `demo`、`openai-image`、其他 `codex-dev` 阶段和产物格式均未改变。第 7 次真实 `detail_vc` 已于 2026-07-15 17:01:40 至 17:09:53 成功，正式详情配置已通过 schema 与业务复核并排他落盘，真实路由前进至 `needs_final_prompts`。临时真实服务已关闭，三个作用域的真实执行开关均为空；`final_prompts` 未执行，必须等待用户另行明确批准。
