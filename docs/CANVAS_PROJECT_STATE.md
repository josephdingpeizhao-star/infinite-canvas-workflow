...
>
> 最后更新：2026-07-16（用户批准按 1+5+8 完成 14 张；第一批 `main_01` 请求成功返回 PNG，但实际为 `1254x1254`，不符合锁定的 `1024x1024`，已按失败规则停止后续两批、重试、QC 与 repaired）。
...
1. **②b 当前停在第三次 `main_01` 尺寸验收失败后**：用户已经批准按 `1+5+8` 完成 14 张，但第一批返回的有效 PNG 实际为 `1254x1254`，不符合任务锁定的 `1024x1024`。后续 5+8 张未开始，事件 47 行，正式 renders 仅保留这一张现场证据。技术路由显示 `needs_qc_reports` 只是因为检测到任一 render，不代表 14 张业务任务完成。任何继续方案必须先决定是接受供应端尺寸、增加受控尺寸归一化，还是更换能严格返回目标尺寸的通道；在新方案批准前禁止继续请求。
...
3. **继续禁止现场渲染与 QC**：当前允许保留 47 行脱敏事件、尺寸不符的 `main_01.png` 和本账本失败记录；不得重提 `run: renders`/`retry: renders`，不得裁剪、缩放、执行 QC 或 repaired，也不得再改 manifest、schema、Skill、fork、提示词或正式上游产物。只有用户批准新的尺寸处理方案和对应成本后才能继续。
