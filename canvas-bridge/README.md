...
2. **原图门**：至少一张原始素材直接连给同一台机器；文件名、类型、大小、来源声明和路径边界全部合法。MU-01 批量入口单次允许选择 1–12 张产品原图，超过 12 张整批拒绝、不部分接受。浏览器先对全部原始 `File` 算 SHA-256，并复用 NC-01 的同哈希前置门禁；批内重复、与已连产品原图重复、信息卡未连接工作流机器或连接不唯一时，都在创建图片节点和发号前以人话整批拒绝。
...
MU-01 不新增上传、登记或服务端管线。批量入口在本地入库和发号前先从既有 17373 健康端点确认 `batch_intake` 工人在岗；服务未开、工人停止或画布重连时不创建节点、不连线、不发号，也不启动计时。健康通过后，每张图仍逐一走原 localforage 原字节凭证通道；全部本地步骤成功才一次写回图片节点与“图片 → 该信息卡唯一关联工作流机器”的血缘连线，并调用原登记入口一次。其后的 8 秒接单确认、17372 逐图交付、失败即停和无自动重试行为保持不变；任一图在写回前失败时，整批不出现部分节点或连线，并清理调用方已经取得的本地凭证。
...
- 单文件最多 64 MiB，服务端单批最多 512 MiB、最多 100 张；MU-01 画布入口另以单次 12 张作为更严格的交互上限。PNG、JPEG、WebP、GIF、BMP 还要同时通过文件类型与文件头核对。
...
- 回收入口为 `python canvas-bridge/batch_recycle_cli.py recycle <批次号>`；还原入口为同一 CLI 的 `restore <批次号>`。回收先在主仓事件账本追加 `batch_recycled`，再清理该批次可证明归属的画布节点，最后把整个工作区顶层目录一次改名到同级 `_回收站/<批次号>__<UTC时间戳>`。账本和 batch manifest 永远留在主仓原位。
...
- 还原先把目录单次改名回 manifest 声明的原位置，成功后才追加 `batch_restored`；若进程在两步之间退出，重跑只补事件。还原不自动重建画布节点，需按原工作流入口重新投影。系统不实现自动物理删除；需要彻底清空时只能人工处理 `_回收站`。
