...
def variable_config(
    *,
    ...
    base = {
        "页面任务": page_task,
        "绑定角度槽位": bound_angle,
        "角度适配原则": "以绑定白底图的产品朝向、俯仰关系、杯口可见程度、杯身外壁透视、杯底可见关系和杯柄空间关系完成本张任务，不改变产品角度。",
        "产品角度依据": "本张产品角度以绑定角度槽位对应的白底产品图为唯一依据；其他图片只用于确认身份、结构和风格。",
        "产品颜色依据": "商品本体颜色以绑定白底图为唯一颜色参照；风格母版、背景、道具和环境光只允许造成真实局部明暗，不得改变雾蓝、灰蓝、青蓝、乳白渐变釉及蓝色喷点关系。",
        "产品结构关系依据": "杯口、外扩杯沿、矮宽杯身、圆环杯柄、窄底足和釉面颗粒/流釉纹理",
        "辅助参考图调用": "无多角度单件合并参考图；白底图组只作角度入库和结构辅助，不改变本张绑定角度与颜色依据。",
        "展示重点": "保持雾蓝到乳白的渐变釉、蓝色喷点、深色杯内釉、外扩杯口、矮宽杯身和圆环杯柄清晰可识别。",
        "构图方式": composition,
        ...
        "风格贴合锚点调用": ["米白桌面和浅暖墙面", "柔和侧上方自然光", "绿植虚化背景", "浅色布料自然褶皱", "木勺/蓝莓/面包等生活化弱道具"],
        "风格精简描述": "暖米白生活化商业摄影；浅米白桌面和背景墙，柔和侧上方自然光，前景可有少量虚化绿叶或蓝莓，中景产品清晰，背景使用玻璃杯、绿植枝条、浅色书本或餐盘形成层次；真实接触阴影，不退化为纯白棚拍。",
        "道具密度等级": "丰富生活场景",
        "背景层次配置": "前景少量虚化绿叶或蓝莓，中景产品主体与浅色布料，背景为柔化绿植、玻璃杯、浅色书本或餐盘；结构说明图可简化但至少保留两层空间。",
        "内容物状态": "默认空杯；使用场景图可加入咖啡、茶或浅色饮品液面，但不得改变杯体结构，不得生成溢出或违背重力的液体。",
        "手持交互声明": "本张图不启用手持场景",
        "动态手持样式参考图调用": "无",
        "背景与光线": "延续暖米白生活化商业摄影；柔和侧上方自然光，釉面高光真实，杯底与桌面接触阴影明确，背景/道具不抢主体。",
        "文字信息": "本张图不设置文字信息" if text == "无" else "小面积中文文字，服务本张页面任务，不遮挡杯口、杯沿、杯身渐变釉或杯柄。",
        "中文营销文案": text,
        "文字渲染要求": "无" if text == "无" else "只渲染【中文营销文案】列出的简体中文，深蓝灰或深灰色，清晰无乱码，不遮挡产品。",
        "道具生成": prop_direction,
        "真实感要求": "真实商业摄影质感；陶瓷釉面、高光、颗粒纹理、接触阴影和景深可信；避免塑料感、漂浮感、文字漂浮和 AI 融化边缘。",
        "风格防退化检查": "保留暖米白桌面、浅色布料、绿植/玻璃杯/餐盘等前中后景层次和真实接触阴影；不得退化为孤立白底棚拍。",
        "禁止事项": "不要改变杯型、颜色、釉面、喷点、杯口外扩关系、杯柄形态或底足；不要添加杯盖、吸管、杯碟、勺子为商品本体；不要生成多个杯作为销售件数承诺；不要标注未提供的尺寸容量。",
    }
    if extra:
        base.update(extra)
...
def main() -> int:
    ...
    if len(white_bg) != 6:
        raise SystemExit(f"expected 6 white-bg JPG files, found {len(white_bg)}")
    if not style_refs:
    ...
    manifest["notes"] = "Product inputs rebuilt from desktop 杯类 folder on 2026-06-15. Batch remains single-product."
    write_json(MANIFEST_PATH, manifest)
    write_json(paths["manifests"] / "batch_manifest.json", manifest)

    slot_map = {
        "1S0A9957.JPG": ("white_bg_01", "角度槽位 A：正面微俯视", "高位微俯视，杯柄在左后侧，杯口和深色杯内区域明显可见，杯身正面与底足部分可见。"),
        "1S0A9961.JPG": ("white_bg_02", "角度槽位 B：45°斜侧视", "侧前方中低机位，杯柄位于左侧，杯身高度、外扩杯口和窄底足关系清楚。"),
        "1S0A9964.JPG": ("white_bg_03", "角度槽位 A：正面微俯视", "高位微俯视，杯口椭圆、杯内空间、杯身前方渐变釉和左侧杯柄同时可见。"),
        "1S0A9967.JPG": ("white_bg_04", "角度槽位 B：45°斜侧视", "斜侧中景，杯柄在左侧，杯口内缘和杯身外壁渐变釉清楚，底部支撑可见。"),
        "1S0A9968.JPG": ("white_bg_05", "角度槽位 D：侧面低角度", "侧面低角度倾向，杯身高度、外扩杯沿、圆环杯柄轮廓和窄底足支撑关系突出。"),
        "1S0A9969.JPG": ("white_bg_06", "角度槽位 B：45°斜侧视", "斜侧中景，杯柄和杯身外壁比例清楚，杯口内侧弱可见，适合生活化主图。"),
    }
    slot_lookup = {asset_id: {"slot": slot, "note": note, "file": image} for image in white_bg for asset_id, slot, note in [slot_map[image.name]]}
        ...
        [
            {
                ...
                "bound_angle_slot": "",
                "component_id": "",
                "notes": "Warm cream lifestyle e-commerce reference with Chinese title, greenery, glass cup, cloth, wooden spoon, blueberries, bread plate and soft natural light.",
            },
            {
    ...
    identity = {
        ...
        "identity": {
            ...
            "auxiliary_reference_usage": {
                ...
                "confirmed_scope": "无",
                "unconfirmed_content": "背面细节、底部完整结构、真实尺寸、容量、重量无法确认。",
                "conflict_or_risk": "未发现明显冲突；白底图组仅用于单件结构辅助理解和角度入库，不固定后续所有出图角度。",
            },
            "core_shape": "矮宽杯身，杯口外扩，杯壁向底部轻微收窄，底部为较窄的平底/底足；杯柄为左侧圆环形把手，上下连接点贴近杯身，整体没有杯盖、吸管、杯碟、勺或托盘。",
            "visual_proportions": "杯口宽于底部，杯身高度低于常见直筒马克杯，杯口椭圆在微俯视角下明显；圆环杯柄高度接近杯身高度的中上段，杯柄厚度较圆润。",
            "true_dimensions": {
                "source": "无法确认",
                "confidence": "低",
                "height_cm": "无法确认",
            ...
            "color_and_material": "亮面陶瓷釉。上半部和杯内为偏深的蓝灰、雾蓝、青蓝混合釉色，杯口边缘有蓝紫灰过渡；下半部逐渐过渡为乳白/灰白釉面；表面分布蓝色、灰蓝色喷点和流釉颗粒。产品颜色最终以每张绑定白底图为唯一视觉参照。",
            "texture_and_surface": "杯体釉面有明显手作流釉感、颗粒喷点、局部垂流痕迹和烧制不均匀的自然过渡；表面整体为亮面陶瓷反光，不是塑料、玻璃或金属。",
            "pattern_and_decoration": "无独立图案、文字、品牌标识或金边装饰；装饰来自釉色渐变、喷点和流釉纹理。",
            "structural_details": "杯口外扩且边缘略有手作不完全规则感；杯内壁深蓝灰釉色明显；杯柄为圆润环形，与杯身连接处有釉色连续过渡；底部为较窄平底，完整底面不可见。",
            "angle_usage_rule": "本档案不固定产品展示角度。后续生成主图、详情图或场景图时，产品角度和颜色均以当次绑定白底图或角度入库表中绑定槽位对应白底图为唯一依据。",
            "must_keep": ["矮宽外扩杯身", "外扩杯口和厚实杯沿", "圆环杯柄", "窄底足/平底支撑", "雾蓝/蓝灰到乳白渐变釉", "蓝灰喷点和流釉颗粒", "亮面陶瓷材质", "无杯盖、无吸管、无杯碟、无金边、无独立图案"],
            "allowed_changes": ["背景", "桌面", "光线", "道具", "文字位置", "构图方式", "产品在画面中的位置和占比", "在角度入库表允许范围内切换绑定白底角度"],
            "prohibited_inventions": ["不要添加杯盖、吸管、杯碟、托盘、勺子或金边为商品本体", "不要把圆环杯柄改成方形或细长把手", "不要把矮宽杯身改成高直筒杯", "不要把雾蓝渐变釉改成纯白、纯蓝、纯绿或其他颜色", "不要删除喷点和流釉纹理", "不要添加品牌字母、图案或花纹", "不要生成多个杯作为销售套装承诺", "不要从白底图推断厘米尺寸、容量或重量", "不要在尺寸无法确认时启用手持或强比例道具", "不要为了风格复现把产品重绘成参考图里的其他杯型"],
            "product_lock_description": "请严格保持产品本身不变：单只雾蓝陶瓷马克杯，矮宽杯身、外扩杯口、厚实杯沿、圆环杯柄、较窄平底/底足，亮面陶瓷釉面呈深蓝灰/雾蓝/青蓝到乳白的自然渐变，并带蓝灰喷点与流釉颗粒。原图没有杯盖、吸管、杯碟、勺、托盘、金边、品牌字母或独立图案，不得增加。产品展示角度和商品本体颜色以当次绑定白底图为唯一依据；只允许改变背景、光线、道具、构图和文字排版，不允许重新设计产品，不允许根据白底图推断精确尺寸或容量。",
            "negative_prompt_constraints": "不要改形、改色、改材质；不要把矮宽外扩杯身改成高直筒杯；不要改变圆环杯柄、外扩杯口、窄底足、雾蓝到乳白渐变釉、喷点和流釉颗粒；不要新增杯盖、吸管、杯碟、勺、托盘、金边、品牌字母或图案为商品本体；不要生成多个杯作为套装承诺；不要标注未提供的尺寸、容量或重量；不要乱码、错字、手部畸形、漂浮、穿模、边缘融化或塑料质感。",
        },
        "missing_information": ["真实高度", "最大宽度或外径", "口径", "底径", "容量", "重量", "完整底部细节"],
        "blocked_reasons": [],
        "notes": "Visual facts are based on the six white-background product images. Real dimensions and capacity were not provided and are not inferred.",
    }
    ...
    style_master = {
        ...
        "style_master": {
            "overall_positioning": "适合淘宝天猫杯具主图、详情页首屏和生活场景模块的暖米白生活化商业摄影风格，可承载少量中文标题和卖点图标。",
            "layout": "主体位于下半区或右下中景，左上/上方留出中文标题区；可使用上下分区或轻微左右分区，但单张任务可调整。",
            "background_rules": "浅米白桌面与浅暖背景墙，真实空间背景而非纯色棚拍；可加入布料、玻璃杯、浅色书本、餐盘和绿植虚化，保留自然褶皱、桌面细微纹理和真实接触阴影。",
            "color_rules": "整体为暖米白、浅灰白、柔和绿植色、低饱和蓝灰点缀；对比度中低，明度偏高，不能用高饱和霓虹色或强烈冷色滤镜影响商品本体颜色。",
            "lighting_rules": "柔和侧上方自然光，阴影边缘柔，产品釉面高光真实但不过曝；背景有轻微明暗层次和自然景深。",
            "subject_rules": "商品保持视觉中心，主图产品占比约 55%-72%，详情图可按任务做中近景或局部；不得因风格而改变产品角度、结构或颜色。",
            "prop_rules": "生活化弱道具可包括浅色布料、绿植枝条、玻璃杯、木勺、蓝莓、面包/浅色餐盘、浅色书本；道具服务层次和饮用场景，不得被误认为商品组成。",
            "text_rules": "参考图含大号中文标题、较小副标题和线性图标卖点。后续可用深蓝灰/深灰中文文字，层级清晰、留白充足；具体文案由变量配置决定，不复制未确认承诺。",
            "negative_rules": ["不要纯白棚拍退化", "不要复杂拼贴或多宫格", "不要强霓虹色", "不要硬阴影", "不要杂乱堆道具", "不要文字遮挡主体", "不要复制未确认的认证/销量/售后承诺", "不要为了风格改变产品身份或角度"],
            "style_fidelity_anchors": ["浅米白桌面和背景墙", "柔化绿色枝叶在背景右上形成虚化层", "浅色布料承托产品并有自然褶皱", "木勺、蓝莓、面包盘等饮用/早餐场景弱道具", "深蓝灰中文标题与线性图标小卖点", "柔和侧上方自然光和真实接触阴影"],
            "reusable_prop_clusters": {
                "must_keep_for_high_fidelity": ["浅米白桌面/背景", "柔化绿植", "浅色布料", "深蓝灰中文标题或小面积信息区"],
                "replaceable_same_class": ["木勺", "蓝莓", "面包/浅色餐盘", "玻璃杯", "浅色书本"],
                "optional": ["前景虚化叶片", "少量散落小食物"],
            },
            "background_layers": {
                "foreground": "可有少量虚化绿叶或蓝莓，低干扰。",
                "midground": "产品主体清晰，位于浅色布料或桌面上。",
                "background": "浅暖墙面、柔化绿植、玻璃杯或浅色书本形成生活空间。",
            },
            "prop_density_level": "丰富生活场景",
            "content_and_usage_state": "参考图杯内为空或深色内壁为主；使用场景图可加入咖啡/茶/浅色饮品液面，内容物属于使用场景元素，不属于产品身份。",
            "text_inheritance": "参考图含文字。后续可添加小面积中文标题/卖点，但不得复制未确认的具体承诺；如页面任务不需要文字，可不加。",
            "anti_degradation_rules": "不得退化为纯白或纯灰背景；不得删除主要前中后景层次；不得把生活化桌面改成孤立白底棚拍；不得用大标题遮挡产品。",
            "compact_style_master": "暖米白生活化杯具电商摄影风格：浅米白桌面与背景墙，柔和侧上方自然光，产品置于布料或桌面中景，前景可有少量虚化绿叶/蓝莓，背景保留柔化绿植、玻璃杯、浅色书本或餐盘。道具密度为丰富生活场景，但不得堆叠或遮挡主体。可用深蓝灰中文标题与小面积卖点信息；具体文案服从变量配置。该风格只约束背景、光线、留白、文字气质和道具密度，不覆盖产品身份、角度、颜色和尺寸规则。",
        },
        "missing_information": [],
    ...
    for image in white_bg:
        asset_id, slot, note = slot_map[image.name]
        if "A" in slot:
            natural = ["杯口外扩关系", "杯内深色釉面", "杯身渐变釉", "杯柄连接位置"]
        ...
        angle_slots.append(
            {
                ...
                "not_to_force": avoid,
                "usable_for": tasks,
                "main_image_fit": "适合" if "B" in slot else "勉强适合",
                "detail_image_fit": "适合",
                "risk_notes": "无明显风险" if image.name not in {"1S0A9968.JPG"} else "侧面低角度倾向但仍可见杯口，避免要求完整顶部结构。",
    ...
    angle_inventory = {
        ...
        "missing_angle_slots": ["角度槽位 C：顶部俯视"],
        "notes": "All provided white-background images are single-product mug references. No set-product workflow is enabled. Top-down slot C is missing and must not be forced.",
    }
    ...
    main_configs = [
        variable_config(config_id="main_01", output_type="main", page_task="主图第一印象与商品识别", bound_angle=ba("white_bg_02"), composition="产品居中偏下完整展示，左上保留小面积标题区，背景延续浅米白生活化场景。", product_position="居中偏下，杯柄朝左，杯口和杯身完整可见。", product_ratio="约 62%-68%，突出单只马克杯主体。", text="雾蓝马克杯", prop_direction="1-3 个弱道具：浅色布料、虚化绿植或玻璃杯；不遮挡杯口、杯沿、杯身渐变釉和杯柄。"),
        variable_config(config_id="main_02", output_type="main", page_task="生活化饮用氛围主图", bound_angle=ba("white_bg_06"), composition="产品偏右下中景，左侧和上方保留柔和留白，前景少量蓝莓或绿叶虚化，背景可有玻璃杯与绿植。", product_position="偏右下，杯柄仍按绑定白底图朝左，不旋转。", product_ratio="约 55%-62%，保留桌面生活层次。", text="无", prop_direction="3-5 个生活化弱道具：布料、蓝莓、木勺、玻璃杯、绿植；道具分布在前景/背景，不堆在杯身周围。"),
        variable_config(config_id="main_03", output_type="main", page_task="釉色纹理主视觉", bound_angle=ba("white_bg_01"), composition="中近景强调杯口、杯内深色釉和杯身喷点，背景简化为浅米白桌面与虚化绿植。", product_position="居中略偏右，杯口和上半部釉色为视觉重点。", product_ratio="约 65%-72%，允许局部接近但不裁掉杯柄连接点。", text="自然流釉纹理", prop_direction="0-2 个弱道具：浅色布料或背景绿植；不得遮挡杯口、杯内釉面和喷点纹理。"),
        variable_config(config_id="main_04", output_type="main", page_task="杯柄与侧面轮廓展示", bound_angle=ba("white_bg_05"), composition="侧面低角度生活化主图，杯身侧面与圆环杯柄清晰，右上保留小面积文字。", product_position="居中偏左，杯柄完整可见，底足与桌面接触清晰。", product_ratio="约 60%-68%，完整保留杯口、杯柄和底足。", text="宽口矮身", prop_direction="1-3 个弱道具：浅布、木勺、背景书本；全部低存在感，不形成尺寸暗示。"),
        variable_config(config_id="main_05", output_type="main", page_task="克制留白点击主图", bound_angle=ba("white_bg_04"), composition="产品居中，浅米白背景留白更大，使用少量图标式文字但不做复杂信息图。", product_position="居中，杯柄朝左，杯口轻微可见。", product_ratio="约 58%-64%，产品边界清楚。", text="手作釉色", prop_direction="0-2 个极弱道具：布料或一小枝绿植；保持主图干净，不遮挡主体。"),
        variable_config(config_id="main_06", output_type="main", page_task="风格复现型品牌氛围主图", bound_angle=ba("white_bg_03"), composition="贴合参考图的上方标题区和下方产品区，产品置于布料上，背景含柔化绿植、玻璃杯和浅色书本。", product_position="偏右下，杯口和杯柄完整可见。", product_ratio="约 55%-62%，保留前中后景层次。", text="静谧雾蓝", prop_direction="4-6 个非产品元素：布料、绿植、玻璃杯、木勺、蓝莓、浅色书本；作为风格锚点分层出现，不遮挡产品。"),
    ]
    ...
    detail_configs = [
        variable_config(config_id="detail_01", output_type="detail", page_task="模块01 首屏 · 主视觉与卖点承接", bound_angle=ba("white_bg_02"), composition="首屏左右轻分区，产品在右下中景完整展示，左上使用商品名与短副标题承接主图。", product_position="右下主体，杯柄朝左，杯口和杯身完整可见。", product_ratio="约 58%-65%。", text="雾蓝马克杯｜自然流釉", prop_direction="2-4 个生活化弱道具：布料、绿植、玻璃杯、浅色书本；不得遮挡主体。", extra={"标准模块归属": "模块01 首屏 · 主视觉与卖点承接", "买家疑问": "这是不是我刚才点进来的雾蓝马克杯", "尺寸标注信息": "非尺寸标注图，不启用尺寸标注信息", "尺寸标注图规则": "非尺寸标注图，不启用"}),
        variable_config(config_id="detail_02", output_type="detail", page_task="模块02 核心卖点证明", bound_angle=ba("white_bg_01"), composition="中近景突出杯口外扩、深色杯内釉、雾蓝渐变和喷点纹理，文字在上方留白。", product_position="居中偏下，杯口与上半部釉色清晰。", product_ratio="约 66%-72%。", text="雾蓝渐变釉", prop_direction="0-2 个弱道具，优先保留浅色布料或虚化绿植；细节不被道具干扰。", extra={"标准模块归属": "模块02 核心卖点证明", "买家疑问": "核心视觉特点在哪里", "尺寸标注信息": "非尺寸标注图，不启用尺寸标注信息", "尺寸标注图规则": "非尺寸标注图，不启用"}),
        variable_config(config_id="detail_03", output_type="detail", page_task="模块03 使用场景与方法", bound_angle=ba("white_bg_06"), composition="生活化早餐/饮品桌面场景，可加入真实液面但不改变杯体；产品为中景主体，背景柔化。", product_position="居中偏右，杯柄朝左，杯口可见。", product_ratio="约 52%-60%。", text="咖啡与茶饮日常", prop_direction="3-5 个使用场景道具：蓝莓、面包盘、木勺、绿植、布料；均为非商品道具，不暗示套装。", extra={"标准模块归属": "模块03 使用场景与方法", "买家疑问": "日常使用场景是否自然", "内容物状态": "可加入咖啡或茶的真实液面，液面服从杯口角度和重力；不标注容量。", "尺寸标注信息": "非尺寸标注图，不启用尺寸标注信息", "尺寸标注图规则": "非尺寸标注图，不启用"}),
        variable_config(config_id="detail_04", output_type="detail", page_task="模块04 细节实拍与材质工艺", bound_angle=ba("white_bg_03"), composition="细节近景但保留足够杯型识别，强调喷点、流釉颗粒、杯沿厚度和亮面陶瓷高光。", product_position="居中，杯口和杯身上半部为重点，杯柄连接点不被裁掉。", product_ratio="约 68%-74%。", text="喷点与流釉颗粒", prop_direction="0-1 个极弱背景道具；材质说明图以产品细节为主，不堆道具。", extra={"标准模块归属": "模块04 细节实拍与材质工艺", "买家疑问": "釉面与细节是否清楚可信", "尺寸标注信息": "非尺寸标注图，不启用尺寸标注信息", "尺寸标注图规则": "非尺寸标注图，不启用"}),
        variable_config(config_id="detail_05", output_type="detail", page_task="模块05 规格尺寸与容量", bound_angle=ba("white_bg_04"), composition="非精确尺寸模块：用完整杯身和杯口展示器型比例，不输出厘米、毫升或克数；文字只说明未含数值的器型信息。", product_position="居中偏左，右侧留出小面积说明区域。", product_ratio="约 58%-64%。", text="宽口矮身｜尺寸以实物为准", prop_direction="0-2 个弱道具，避免书本、手机、手部、小勺等强比例参照；不得暗示具体大小。", extra={"标准模块归属": "模块05 规格尺寸与容量", "买家疑问": "杯型比例大致如何", "尺寸标注信息": "尺寸来源无法确认；不允许渲染任何厘米、毫升或克数标注。", "尺寸标注图规则": "不画尺寸线、不画箭头、不输出精确数值；只可做非数值器型比例展示。"}),
        variable_config(config_id="detail_06", output_type="detail", page_task="模块06 质感可信视觉呈现", bound_angle=ba("white_bg_05"), composition="侧面低角度中近景，强调杯身侧面渐变釉、杯柄轮廓、底足接触阴影和真实陶瓷反光。", product_position="居中略偏左，底足与桌面接触清晰。", product_ratio="约 62%-70%。", text="亮面陶瓷釉感", prop_direction="1-3 个材质对比弱道具：布料、木勺、玻璃杯背景；不得遮挡杯柄和底足。", extra={"标准模块归属": "模块06 质感可信视觉呈现", "买家疑问": "实物质感是否可信", "尺寸标注信息": "非尺寸标注图，不启用尺寸标注信息", "尺寸标注图规则": "非尺寸标注图，不启用"}),
        variable_config(config_id="detail_07", output_type="detail", page_task="模块07 决策辅助与场景想象", bound_angle=ba("white_bg_06"), composition="桌面摆放氛围图，产品与浅色布料、绿植、书本、蓝莓形成日常摆放想象，文字小面积靠上。", product_position="偏右下，产品清晰完整。", product_ratio="约 50%-58%。", text="日常桌面一角", prop_direction="3-5 个氛围道具：绿植、浅色书本、蓝莓、木勺、玻璃杯；不形成商品组成误解。", extra={"标准模块归属": "模块07 决策辅助与场景想象", "买家疑问": "它放在日常桌面中是否协调", "尺寸标注信息": "非尺寸标注图，不启用尺寸标注信息", "尺寸标注图规则": "非尺寸标注图，不启用"}),
        variable_config(config_id="detail_08", output_type="detail", page_task="模块08 收尾氛围与风险克制", bound_angle=ba("white_bg_02"), composition="克制收尾图，产品完整清晰，背景保留柔化绿植和暖米白空间，文字可无或极小。", product_position="居中偏下，保留完整轮廓。", product_ratio="约 55%-62%。", text="无", prop_direction="2-4 个低干扰道具：布料、绿植、玻璃杯、蓝莓；不生成售后、质保、价格、促销或资料缺失说明。", extra={"标准模块归属": "模块08 收尾氛围与风险克制", "买家疑问": "最后的整体视觉印象是否真实克制", "尺寸标注信息": "非尺寸标注图，不启用尺寸标注信息", "尺寸标注图规则": "非尺寸标注图，不启用"}),
    ]
