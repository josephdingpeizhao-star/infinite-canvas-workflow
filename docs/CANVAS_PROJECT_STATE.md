...
>
> 最后更新：2026-07-16（用户接受供应端原生正方形尺寸并批准剩余 13 张；续跑成功生成 `main_02` 后，`main_03` 遭遇 HTTP 502，已按不重试规则停止，现有 2/14 张，详情图、QC 与 repaired 均未开始）。
...
1. **②b 当前停在 `main_03` HTTP 502 后**：用户已接受供应端原生正方形尺寸；`main_01` 与 `main_02` 均为有效 `1254x1254` PNG。剩余 12 张未生成，事件 49 行；续跑批次在成功 1/计划 5 后因 `main_03` HTTP 502 按不重试规则停止，详情图批次完全未开始。技术路由显示 `needs_qc_reports` 只是因为检测到 render，不代表 14 张业务任务完成。任何继续调用都必须重新批准失败恢复方案及对应成本。
...
3. **继续禁止现场渲染与 QC**：当前允许保留 49 行脱敏事件、`main_01.png`、`main_02.png` 和本账本失败记录；不得重提 `run: renders`/`retry: renders`，不得裁剪、缩放、执行 QC 或 repaired，也不得再改 manifest、schema、Skill、fork、提示词或正式上游产物。只有用户批准新的失败恢复方案和对应成本后才能继续。
