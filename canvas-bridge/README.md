...
5. `shuiping_20260712` 已完成 prompts-only 门禁与模型探测；前两次获批的单张 `main_01` 调用分别以非标准 HTTP 403 和响应读取超时安全失败。第三次获批调用在 111 秒后返回 PNG，但实际尺寸为 `1254x1254`，不满足任务锁定的 `1024x1024`，因此后续 5+8 张按门禁停止。事件账本现为 47 行，正式 renders 目录保留这一张未通过尺寸验收的 `main_01.png`。后续现场闸门见 `docs/CANVAS_PROJECT_STATE.md` §8，不能自动连续执行。

## 状态

阶段 1（只读实时投影）、阶段 2（布局持久化）、阶段 3（受控编辑，`--apply-edits`）、阶段 4（执行接入，`--serve` 运行台）均已跑通并有测试与现场验证。`codex-dev` 的 identity、style master、angle inventory、`main_vc`、`detail_vc` 和 `final_prompts` 已完成 `shuiping_20260712` 真实验收；正式目录包含 14 份 JSON、14 份同名 Markdown 和两份索引。生产图片链已完成 prompts-only 门禁、14 项任务组装、裸 API 基址兼容、固定客户端标识和可配置无数据等待上限，当前全仓 238 项测试通过。第三次单张 `main_01` 请求已成功返回文件，但供应端实际输出 `1254x1254`，未达到锁定的 `1024x1024` 验收尺寸；后续两批未开始。真实路由因检测到一张 render 已转为 `needs_qc_reports`，但业务批次仍只有 1/14 张且未完成；事件 47 行，ComfyUI 作业与 repaired 均为 0，QC 未执行，渲染继续冻结。
