...
- `workflow_production_service.py`：M2-b 后台编排。顺序复用现役 `codex-dev` 六工序、`image-production / integrity` 和 `image-production / renders`，机器只显示人话进度，不投影九工序、运行台或日志。任一步失败即停且不自动重试；主/详情变量配置的受控语义分类可进入原批次失败事件并转成人话。完整性失败只在正式报告确为 fail 且阻塞数为安全整数时记录固定的“N 项阻塞，报告已写入 qc_reports”；浏览器持久化超时只允许精确固定文案受控透出。两类路径都不回显真实路径；报告异常、其他错误及含路径、URL、令牌或密钥的内容仍使用既有笼统文案。部分图片只能用既有 `retry: renders` 重新进三段门禁；路由以 `final_prompt_index.json` 的 `config_id` 清单为目标，合并核对 renders 与 repaired 同名 PNG，全部覆盖后才停在 QC 前。
...
2026-07-22 第三批闸门②首张 `main_01.png` 已完好落盘，但 17373 当时没有暴露 `x-content-sha256`，导致跨源浏览器无法读取校验头并拒绝持久化；后台随后按超时规则停止且未自动重试。主仓随后补齐上述暴露声明和固定超时受控透出；2026-07-23 用户重开画布后，浏览器已自动补导入 `main_01.png`，因此 CORS 修复完成真实浏览器实战验收，未产生新的图片调用或费用。同日又修正 1/14 部分渲染被误判为待质检的阶段路由；真实路径已纯只读验为 `needs_generated_images_before_qc`，剩余 13 张仍等待用户的独立续渲批准。
...
- 闸门执行中只接受用户画布发出的唯一命令；同一页面的机器/批次在一次确认后保持锁定。任何失败立即停，不现场改码、不自动重试、不追加第二条命令；另获批准后须重新打开画布再由用户亲手开始。闸门②固定 `RENDER_MAX_IMAGES=1`；闸门③只在另获批准后处理剩余缺口。
...
- 未开启本次进程的真实执行开关时，`codex-dev` 以稳定码 `real_execution_disabled` 在模型调用前停止。工作台固定显示“本机真实执行开关未开启，本次没有调用模型、没有产生费用。请先关闭工作台窗口，按闸门流程用带开关的命令重新启动工作台，再回到画布重新开始。”；事件账本固定记录“真实执行开关未开启，执行已停止，未自动重试”。该提示不改变零模型调用、零费用、零产物、零自动重试的既有安全语义。
...
M2-a 已按用户批准完成信息卡画布直填、中文批次命名、受控建批、原图保真通道、工作台承载切换和阶段 D 隔离验收。M2-b 阶段 C/D 与三轮离线稳定性修复已完成：真实费用卡、三段门禁后台编排、逐张正式图片上桌并转存浏览器、信息卡风格补登 A、关键工人健康、空答复的正常/备用失败通知和同页单次提交均有自动测试；主仓 402 项、fork 61 项（332 个断言）、Canvas Agent 构建、TypeScript 与生产构建全部通过。专项离线核查确认本次停止发生在第一道产品识别，尚未进入完整性或构造图片执行器；`/v1/images/edits` 与 `/v1/images/generations` 的离线装配测试通过，因此本次失败不是最后图片 API 接口导致，但供应端真实兼容性仍只能在另获批准的出图闸门验证。没有触碰真实用户画布、真实模型、密钥或费用；阶段 E 重跑必须逐闸门另获批准。首批 `shuiping_20260712` 仍保持关账冻结，事件 72 行、路由 `ready`，任何图片追加、覆盖或重生成仍须另行批准。

2026-07-23 最新状态：第三批 `杯子_20260722` 的六道上游与完整性门禁均已通过，6 张主图与人工扩边后的 `detail_01` 已持久化；`detail_02` 再次返回 1024×1536 后按旧政策停止，事件账本为 83 行。用户已拍板用自动无损扩边取代逐张停机等人工，存量清扫、逐张扩边、脱敏事件和 Pillow 缺失回退均已纯离线 TDD 完成，主仓全量 500/500；fork 保持 `d30f3dc` 且零修改。本轮没有启动服务、联网、调用模型、产生费用，也没有改变两份 manifest、真实 `detail_02` 及其审计副本、事件账本、`reports/current_state.*` 或启动器。用户重开工作台并续跑后，现存 `detail_02` 会先清扫为 3:4，再继续后续详情图。
