...
>
> 最后更新：2026-07-17（用户接受 `detail_01` 的供应端 3:4 并批准按 `1+6` 续跑；单张 `detail_02` 传输成功但实际返回 2:3，不符合本轮批准的 3:4，已立即停止且未进入最后六张。正式目录 8 个文件，其中六张主图与 `detail_01` 通过，`detail_02` 保留为尺寸失败证据；QC 与 repaired 均未开始）。
...
| 4b | 生产图片执行链（prompts-only 门禁 + 索引组装 + image-production） | ✅ 离线实现 | 5f98b35、6f9ffcb | 238/238 测试；真实门禁 pass；现场两次单张调用均安全失败，0 张成图 |
...
1. **②b 当前停在 `detail_02` 尺寸验收失败后**：`main_01` 至 `main_06` 六张主图均为有效 `1254x1254` PNG，`detail_01` 的 `1086x1448`（3:4）已由用户接受；相同详情执行链随后把 `detail_02` 返回为 `1024x1536`（2:3），不符合本轮批准的 3:4，因此立即停止。事件 58 行，正式目录 8 个文件，`detail_03` 至 `detail_08` 未落盘。技术路由显示 `needs_qc_reports` 只是因为检测到 render，不代表 14 张业务任务完成。
2. **②b 的详情图比例需要再次决策**：真实结果已经证明供应端在相同请求映射下会分别返回 3:4 与 2:3。下一步只能由用户明确选择“同时接受两种竖版比例并把 `detail_02` 计为通过”，或另行设计能稳定统一比例的方案；不得擅自裁剪、拉伸、改映射或重新生成。
3. **继续禁止现场渲染与 QC**：当前允许保留 58 行脱敏事件、六张通过主图、已接受的 `detail_01.png`、作为尺寸失败证据的 `detail_02.png` 和本账本记录；不得重提 `run: renders`/`retry: renders`，不得裁剪、缩放、执行 QC 或 repaired，也不得再改 manifest、schema、Skill、fork、提示词或正式上游产物。只有用户批准新的比例决策、失败恢复方案和对应成本后才能继续。
