...
| MU-01 | 产品原图批量选择、入画与血缘连线 | ✅ fork 实现与文档入账完成；真人画布验收待用户 | 本轮 fork 功能提交；主仓仅文档提交 | fork 121 项/665 断言（基线 105/521 + 新增 16/144）、TypeScript 与构建通过；主仓 692/692；单次 1–12 张、全批发号前预检、自动连接唯一工作流机器、零部分画布写入；既有测试正文零修改 |
...
- 位置：`D:\dev\infinite-canvas`，分支 `workflow-editor`，MU-01 执行前 @ `67ebdd3`，上游基线 ebd8ae2（2026-07-09 origin/main）。
- **改动登记册：`FORK_NOTES.md`（fork 仓库根目录）**——截至 2026-07-26 共 87 个锚点：#5–#8、#36、#38–#41 为 canvas-agent 兼容；#10–#19 为 M1；#20–#35、#37 为 M2；#45–#56 为 QC 角标、返修投影与收货；#57–#59 为 M3-a；#60–#68 为 NC-01；#69–#75 为 M3-b；#76–#80 为 RC-01；#81–#87 为 MU-01 批量原图入口、纯接线、测试与本机 dist。动 fork 前必读，同步上游后逐条复核。
- MU-01 在 fork 纯新增批量预检/布局合同、页面 hook、多选按钮和独立测试，只最小触碰信息卡挂点及 `project.tsx` 三个纯接线点。单次最多 12 张，全部原始文件先复用 NC-01 同哈希门禁并确认 `batch_intake` 工人健康；通过后逐张复用原 localforage 凭证通道，一次写回节点与“图片 → 唯一工作流机器”连线，再调用原登记入口一次。既有单图、拖拽、风格补登、8 秒接单确认、17372 逐图交付、生产和 Agent 行为不变；零新增依赖，canvas-agent 与主仓生产代码零改动。
