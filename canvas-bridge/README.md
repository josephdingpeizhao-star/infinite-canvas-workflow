...
- `--serve` 的默认执行器仍是 `demo`。`codex-dev` 已注册为可选开发适配器，支持 `identity`、`style_master`、`angle_inventory`、`main_vc`、`detail_vc` 与只产出提示词的 `final_prompts`；`image-production` 已注册为生产图片组合入口，内部复用 `openai-image`，不替换也不改变现有三个适配器的行为。
...
4. 主图请求固定 `1024x1024`。详情图请求仍使用暂定 `1024x1536`（2:3）且不做裁剪、拉伸或后处理；真实 `detail_01` 返回 `1086x1448`，即精确 3:4，说明供应端可能按比例语义而非请求像素返回。该文件因不符合本轮获批的约 2:3 验收口径而保留为失败证据；是否改为接受供应端 3:4 必须由用户另行决定。
5. `shuiping_20260712` 已完成 prompts-only 门禁与模型探测。用户在前序失败后批准按 `1+3+8` 续跑剩余 12 张：`main_03` 单张成功，其后 `main_04` 至 `main_06` 三张成功，六张主图均为有效 `1254x1254` PNG；详情批次写出 `detail_01` 后因实际 `1086x1448`（3:4）不符合获批约 2:3 验收口径而立即停止，未生成 `detail_02` 至 `detail_08`。事件账本现为 55 行，正式 renders 有 7 个文件，其中 6 张主图通过、1 张详情图作为尺寸失败证据保留。后续现场闸门见 `docs/CANVAS_PROJECT_STATE.md` §8，不能自动连续执行。

## 状态

阶段 1（只读实时投影）、阶段 2（布局持久化）、阶段 3（受控编辑，`--apply-edits`）、阶段 4（执行接入，`--serve` 运行台）均已跑通并有测试与现场验证。`codex-dev` 的 identity、style master、angle inventory、`main_vc`、`detail_vc` 和 `final_prompts` 已完成 `shuiping_20260712` 真实验收；正式目录包含 14 份 JSON、14 份同名 Markdown 和两份索引。生产图片链已完成 prompts-only 门禁、14 项任务组装、裸 API 基址兼容、固定客户端标识和可配置无数据等待上限，当前全仓 238 项测试通过。正式 renders 现有六张已通过的 `1254x1254` 主图，以及一张因 3:4 不符合本轮约 2:3 口径而未通过的 `detail_01`；其余七张详情图未生成。真实路由为 `needs_qc_reports`，但 14 张业务任务仍未完成；事件 55 行，ComfyUI 作业与 repaired 均为 0，QC 未执行，渲染继续冻结。
