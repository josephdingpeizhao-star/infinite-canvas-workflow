...
- 现有 QC 继续使用 7 个两图批次和第 8 次汇总。第三批最大估算单批附件为 13,473,190 字节，低于 20 MiB 上限；无需缩图或改变附件策略。
- 本轮纯离线测试为 509/509；真实第三批 QC 尚未执行。日常启动工作台后由用户亲手点击一次即可进入该闸门，成功或失败后都会停机，不自动重试。QC 角标与已收货框属于 M2-c 第二段。
...
## 生产图片执行链（已实现；首批真实 QC 已完成，第三批待用户触发）
...
6. `杯子_20260722` 已完成 14/14 出图但尚无正式 `qc_report.json`。M2-c 第一段已让工作台在 `needs_qc_reports` 路由继续执行 QC，并在 `ready` 后停止；真实 QC 仍由用户事后亲手触发。
...
2026-07-23 最新状态：第三批 `杯子_20260722` 已完成 M2-b 收官，14/14 张正式图片全部持久化上桌；M2-c 第一段的 QC 工作台步进已完成纯离线 TDD，全仓 509/509。第三批真实 QC 仍待用户亲手触发；QC 角标与已收货框属于第二段。`detail_05` 的 936×1248 分辨率差异继续挂 QC 与用户终审。
