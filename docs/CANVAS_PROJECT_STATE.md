...
>
> 最后更新：2026-07-26（MU-02 工具栏“上传素材”多选已在 fork 完成并通过全量自动验证；真人画布验收待用户进行。上传后仍由用户连线决定素材角色，不自动连线、不自动登记，也不新增上传阶段的 SHA 比较门禁。）
...
| MU-02 | 工具栏“上传素材”多选 | ✅ fork 实现与全量自动验证完成；真人画布验收待用户 | fork `8c7a431` | 115 项/585 断言（13 个测试文件）、TypeScript、生产构建与差异检查全绿；忽略区 dist 已按新源码重建 |
