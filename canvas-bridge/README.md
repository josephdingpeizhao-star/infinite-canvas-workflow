...
真实费用确认在同一页面内按“机器 + 批次”锁定一次提交：确认成功写出命令后，即使后台失败也不会在原页面再次生成命令。若已取得新的执行批准，用户须重新打开画布，再亲手点击“开始”并重新确认费用；不存在自动重试。空答复时机器显示“本地 Codex 本轮没有返回内容，机器已停下，未自动重试。”，批次事件格式保持不变，工作台脱敏账本只记录 `execution_failure`、步骤与固定码 `empty_assistant_response`。
...
M2-a 已按用户批准完成信息卡画布直填、中文批次命名、受控建批、原图保真通道、工作台承载切换和阶段 D 隔离验收。M2-b 阶段 C/D 与两轮离线稳定性修复已完成：真实费用卡、三段门禁后台编排、逐张正式图片上桌并转存浏览器、信息卡风格补登 A、关键工人健康、空答复失败和同页单次提交均有自动测试；主仓 401 项、fork 55 项（332 个断言）、Canvas Agent 构建、TypeScript 与生产构建全部通过。没有触碰真实用户画布、真实模型、密钥或费用；阶段 E 重跑必须逐闸门另获批准。首批 `shuiping_20260712` 仍保持关账冻结，事件 72 行、路由 `ready`，任何图片追加、覆盖或重生成仍须另行批准。
