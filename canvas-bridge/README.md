...
- 图片响应优先使用 `data[0].b64_json` 的内联字节；仅当该字段缺失而 `data[0].url` 存在时，才从经安全校验的 HTTPS 链接取回同一次生成的图片，不会重新调用生成端点。下载请求只发送固定 `User-Agent`，不携带生成凭据，最多跟随 3 次且每一跳都重新校验，拒绝协议降级、带凭据地址、非公网 IP、fragment、非 2xx、空响应和超限响应。可选 `OPENAI_IMAGE_DOWNLOAD_MAX_BYTES` 只接受 `1000000` 至 `100000000` 的整数，未设置时默认 `20000000` 字节；下载复用 `OPENAI_IMAGE_TIMEOUT_SECONDS`，单张执行结果 metadata 只记录 `retrieval=inline|url`，不记录链接本身。
