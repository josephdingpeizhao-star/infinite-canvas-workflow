...
- 真实执行默认关闭；只有获得用户明确批准后，才可在该次服务进程中临时设置 `CODEX_DEV_ALLOW_REAL_EXECUTION=1`，该开关不写配置、不持久化。既有 identity/style/angle/main 真实验收历史、`ExecutionRequest / ExecutionResult`、三段门禁、默认 `demo`、`openai-image`、其他 `codex-dev` 阶段和产物格式均未改变。最近一次真实 `detail_vc` 仍是 2026-07-15 14:19:27 至 14:23:37 的校验误判失败；本次修复后没有再次调用。正式详情配置仍不存在，临时真实服务保持关闭，`final_prompts` 未执行。
