...
>
> 最后更新：2026-07-16（用户批准按 `1+3+8` 续跑剩余 12 张；六张主图全部成功，详情批次的 `detail_01` 实际返回 3:4、不符合本轮获批的约 2:3 口径，已立即停机。正式目录 7 个文件，其中 6 张主图通过、1 张详情图保留为尺寸失败证据；QC 与 repaired 均未开始）。
...
1. **②b 当前停在 `detail_01` 尺寸验收失败后**：`main_01` 至 `main_06` 六张主图均为有效 `1254x1254` PNG；详情请求仍使用 `1024x1536`，但供应端实际返回的 `detail_01` 为 `1086x1448`（精确 3:4），不符合本轮获批的约 2:3 口径，因此立即停止。事件 55 行，正式目录 7 个文件，`detail_02` 至 `detail_08` 未落盘。技术路由显示 `needs_qc_reports` 只是因为检测到 render，不代表 14 张业务任务完成。
2. **②b 的详情图比例需要重新决策**：真实结果已经证明供应端会把当前详情请求返回为精确 3:4。下一步只能在“接受 3:4 并把 `detail_01` 计为通过”或“继续要求 2:3 并更换/调整受控方案”之间由用户明确选择；不得擅自裁剪、拉伸、改映射或重新生成。
3. **继续禁止现场渲染与 QC**：当前允许保留 55 行脱敏事件、六张通过主图、作为尺寸失败证据的 `detail_01.png` 和本账本记录；不得重提 `run: renders`/`retry: renders`，不得裁剪、缩放、执行 QC 或 repaired，也不得再改 manifest、schema、Skill、fork、提示词或正式上游产物。只有用户批准新的比例决策、失败恢复方案和对应成本后才能继续。
