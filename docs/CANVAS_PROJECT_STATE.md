...
>
> 最后更新：2026-07-23（M2-c 第一段补丁已完成纯离线 TDD：第三批首次真实 QC 因 7 组共用线程导致上下文累积超窗，现改为 7 个批次线程 + 1 个独立汇总线程；真实重跑待用户重启工作台后亲手触发。M2-c 第二段的 QC 角标与已收货框未开始；`detail_05` 的 936×1248 分辨率差异继续挂 QC/用户终审。）
...
- 位置：`D:\dev\infinite-canvas`，分支 `workflow-editor`，上游基线 ebd8ae2（2026-07-09 origin/main）。MU-01 批量入口曾于 2026-07-26 在 `370ea0e` 落地，同日经真人验收否决后以反提交 `497e947` 全量撤回，代码态回到 RC-01（`67ebdd3` 等效）。
- **改动登记册：`FORK_NOTES.md`（fork 仓库根目录）**——截至 2026-07-26 共 80 个锚点：#5–#8、#36、#38–#41 为 canvas-agent 兼容；#10–#19 为 M1；#20–#35、#37 为 M2；#45–#56 为 QC 角标、返修投影与收货；#57–#59 为 M3-a；#60–#68 为 NC-01；#69–#75 为 M3-b；#76–#80 为 RC-01。MU-01 的 #81–#87 已随 revert 一并撤销，编号自 #81 起留给后续任务复用。动 fork 前必读，同步上游后逐条复核。
- MU-01 撤回后需求修正为 MU-02：画布底部工具栏"上传素材"入口直接支持一次多选（现仅单张），上传后不自动连线、不自动登记，角色仍由用户连线决定，与既有单张行为一致仅 ×N。
