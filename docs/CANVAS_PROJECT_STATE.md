...
用户终审裁决正式记为：**8 张返修全部采纳，本批定位为流程验证批；`detail_06` needs_review 关闭。** 本裁决不等于重新运行 QC，也不授权继续生成、复检或交付；下一次真实运行仍须独立费用闸门。
...
`detail_05` repaired 分辨率为 936×1248，与部分 1152×1536 详情图不同；该事实及较高筒状比例、主体占比和规格说明构图曾进入返修目标。用户已按流程验证批口径采纳本批全部 8 张返修图，不再为该差异追加返修；既有 2:3 无损扩边政策保持不变。
...
画布已收货框按 `config_id` 计数，同图位后放入者替换先前选择，拖出即取消；恰好收满 14 个不同图位后才显示“确认关账”。确认时后端逐项复核来源白名单、批次工作区边界、磁盘实物与 SHA，成功后仅追加一次 `batch_acceptance_closed`，完整记录 14 项选择和固定终审口径。账本已有该事件时拒绝重复关账；关账后制作、质检、返修与返修图上桌均在任何清单改写、事件追加或执行器调用前以“本批次已关账……”拒绝。自动化测试全部使用临时账本，真实第三批 135 行账本仍冻结，真实关账留给用户重启工作台后亲手确认。
