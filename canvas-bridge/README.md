...
- `codex-dev` 的自动测试使用假传输和临时工作区，不启动真实 Codex、不访问网络、不读取真实批次图片。2026-07-14 完成长 JSON 根因修复、第 1 段 `notes` 包装格式纠正和最新“未确认参数”误判的窄范围修复后，全仓 161 项测试通过；`detail_vc` 的四段覆盖、同线程续传、U+FFFD/真实截断最多恢复 2 次、空回复及完整或前部已有语法错误的 JSON 不重试、`notes` 包装最多纠正 1 次、两类次数独立、纠正前后业务配置指纹一致、完整八项手持数量及汇总校验、安全的已确认高度在专用尺寸比例语境中放行、相同数值在裸字段或数值前后/嵌套路径中冒充未确认宽度的拒绝、区间/负数/伪装单位拒绝、业务越界与段号/编号错误不重试以及失败零正式文件均有测试。canvas-agent 同时改为连续解码 Codex stdout 的 UTF-8 数据块，避免中文字符跨块时被替换为 U+FFFD；其 3 项测试和 TypeScript 编译通过。真实执行默认关闭；只有获得用户明确批准后，才可在该次服务进程中临时设置 `CODEX_DEV_ALLOW_REAL_EXECUTION=1`，该开关不写配置、不持久化。既有 identity/style/angle/main 真实验收历史保持不变；本轮未改变 `ExecutionRequest / ExecutionResult`、三段门禁、默认 `demo`、`openai-image`、其他 `codex-dev` 阶段或既有产物格式。最近一次真实 `detail_vc` 仍是 22:16:59 至 22:20:35 因“包含未确认参数”而安全拒绝的调用；离线诊断确认该次属于已确认高度被误判，修复后没有再次调用。正式详情配置仍不存在，临时真实服务已关闭，`final_prompts` 未执行。
...
阶段 1（只读实时投影）、阶段 2（布局持久化）、阶段 3（受控编辑，`--apply-edits`）、阶段 4（执行接入，`--serve` 运行台）均已跑通并有测试与现场验证。2026-07-12 已增加供应商无关的可替换执行器边界，并把 demo 迁移到统一接口；GPT Image 2 适配器仍仅完成离线测试。`codex-dev` 的 identity、style master、angle inventory 和 `main_vc` 已于 2026-07-13 完成 `shuiping_20260712` 真实批次现场验收。正式主图变量配置为 6 项、全部 1:1、恰好 2 项手持，只绑定合格 A/B/C 且固定高度约 25 厘米。真实路线当前停在 `needs_detail_variable_configs`：正式详情配置不存在，`final_prompts` 未执行。画布对业务数据的写入仍仅经由阶段 3/4 的三段门禁白名单通道；执行历史以 `<pid>.events.jsonl` 追加日志为事实来源，renders/repaired 仍均为 0。
