...
- 第三批首次真实 QC 在完成前 5 组后因旧单线程上下文累积超出 258,400-token 窗口而失败；第 6 组未进入 Codex，报告目录零新增、14 张 renders 无损。线程隔离补丁的纯离线测试为 512/512；真实重跑须重启工作台、刷新画布后由用户亲手点击“重新开始”，成功或失败后都会停机，不自动重试。QC 角标与已收货框属于 M2-c 第二段。
...
## 生产图片执行链（已实现；首批真实 QC 已完成，第三批首次失败后待重跑）
...
6. `杯子_20260722` 已完成 14/14 出图但尚无正式 `qc_report.json`。M2-c 第一段已让工作台在 `needs_qc_reports` 路由继续执行 QC，并在 `ready` 后停止；首次真实 QC 因旧单线程上下文累积超窗失败，现已完成 7+1 独立线程补丁，待用户重启工作台后亲手重跑。
...
2026-07-23 最新状态：第三批 `杯子_20260722` 已完成 M2-b 收官，14/14 张正式图片全部持久化上桌；M2-c 第一段的 QC 工作台步进和 7+1 线程隔离补丁已完成纯离线 TDD，全仓 512/512。第三批首次真实 QC 已因旧单线程上下文累积超窗失败，正式报告仍未生成；待用户重启工作台后亲手重跑。QC 角标与已收货框属于第二段。`detail_05` 的 936×1248 分辨率差异继续挂 QC 与用户终审。
