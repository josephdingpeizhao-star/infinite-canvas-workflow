...
>
> 最后更新：2026-07-22（第三批 `杯子_20260722` 于 16:21:17 历史首次完成 `detail_vc`，随后 `final_prompts` 因把详情 `negative_prompt` 的“认证编号”和禁止场景词误当正向事实而于 16:27:31 停止；材质与场景两族止损线同时触发。纯离线 TDD 已完成字段语境级重设计：负面清单整体豁免、材质默认放行且只拒紧邻/修饰商品主体的宣称、正向场景门禁和四路否定识别保留。三份真实夹具硬锚点全部满足，全仓由 454 项增至 468 项并全部通过。）。
