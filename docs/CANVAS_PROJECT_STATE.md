...
1. **离线诊断首次 `final_prompts` 失败**：2026-07-15 18:10:24 至 18:12:37 的一次性真实验收因“主图最终提示词未保留手持状态”安全失败，本轮没有重试或第二条命令，正式提示词与临时半成品均不存在，真实路由保持 `needs_final_prompts`。下一步只能先做离线只读诊断并形成新方案；在完成离线验证和用户重新明确批准前，不得再次真实调用，不得复用失败 thread 正文绕过正式运行台落盘。自由文本语义门禁的既有止损原则继续保留：若同类误判达到止损条件，不再追加正则修补，改为提出结构化字段与枚举值校验的重设计方案并另行申请批准。
